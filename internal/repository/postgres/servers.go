...
func (s *Storage) CreateServer(ctx context.Context, serverModel *models.Server) error {
	...
	_, err = s.pool.Exec(
		ctx,
		`INSERT INTO servers (id, name, host, port, username, auth_type, auth_value, os_type, status, managed_by, created_at, updated_at)
...
func (s *Storage) GetServerByID(ctx context.Context, id string) (*models.Server, error) {
	row := s.pool.QueryRow(
		ctx,
		`SELECT id, name, host, port, username, auth_type, auth_value, os_type, status, managed_by, created_at, updated_at
		 FROM servers
		 WHERE id = $1`,
		id,
	)
